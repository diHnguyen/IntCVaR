...
@objective(m, Max, α - sum(p[i]*z[i] for i = 1:length(p)) )#w - sum(s[k] for k=1:length(s))/length(s) )
global x_sol = []
global α_sol = 0
global z_sol = []
global x_now = []
...
println("BasicAlgE_V3_"*dataSet, "; Ins ", Ins, "; β ",β,"; Time ", total_time, "; LB ", LB, "; α_sol ",α_sol,"; x_sol ", findall(x_sol.==1),"; Cells ", nrow(df_cell), "; Iter ", iter, "; W ", LB_w, "; Cuts ", numConv)

timesFile = open("/Users/din/Desktop/IntCVaR/OutputFile/BasicAlgE_V3_"*dataSet*".txt", "a")
