...
#     println("Inside function")
    ...
    temp_df = filter!(row -> row.gL >= max_g, temp_df)
...
#     println("WHILE")
    global iter
    global stopping_cond 
    global total_time
    #iter = 0
    global x_count 
    global K_not 
    global K_delta2 #Separate cells passing O.C.2 from skipped cells
    global Freeze1_trigger 
    global last_x
    global MP_obj
    global con_num
    stopping_cond = 0
...
#     println("K_not length = ", K_not , " out of ", length(p))
...
#                     push!(K,newCell)
    ...
    #                                 if k == 1
    #                                 f = open(outfile, "a")
    #                                     println(f,"Constraint ", i)
    #                                     println(f,"Check constraint: ",find(df[:ARCS_USED][i].==1))
    #                                 close(f)
    #                                 end
                        end
                    ...
                    if (k in K_not) == false
                        push!(K_delta2,k) #CAREFUL THIS IS NOT(!!) NOREP
                    end
